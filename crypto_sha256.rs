...
impl SHA256Hasher {
    ...
    pub fn hash(data: &[u8]) -> [u8; 32] {
        ...
        let mut padded = Self::pad_message(data);
        let chunks = padded.chunks_exact(64);
        ...
        for chunk in chunks {
            ...
            for i in 16..64 {
                ...
                w[i] = w[i].wrapping_add(w[i-15].rotate_right(7) ^ w[i-15].rotate_right(18) ^ (w[i-15] >> 3));
            ...
            for i in 0..64 {
                ...
                let ch = (e & f) ^ (!e & g);
                let temp1 = hi.wrapping_add(s1).wrapping_add(ch).wrapping_add(Self::K[i]).wrapping_add(w[i]);
